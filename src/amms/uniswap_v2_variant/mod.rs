...
impl UniswapV2Pool {
  ...
  pub fn new(address: Address, factory_address: Address, fee: usize, stable: bool) -> Self {
      Self {
          address,
          factory_address,
          fee,
  ...
  pub fn get_amount_out(&self, amount_in: U256, reserve_in: U256, reserve_out: U256, token_in: Address) -> U256 {
      if amount_in.is_zero() || reserve_in.is_zero() || reserve_out.is_zero() {
          return U256::ZERO;
      }

      // Remove fee from amount received first
  ...
  /// Calculates the k value for stable pools
  fn calculate_k(&self, x: U256, y: U256, decimals_x: u8, decimals_y: u8) -> U256 {
      if self.stable {
          let x_normalized = x * U256::from(10).pow(U256::from(18 - decimals_x));
          let y_normalized = y * U256::from(10).pow(U256::from(18 - decimals_y));
          
          let a = x_normalized * y_normalized / U256::from(10).pow(U256::from(18));
          let b = x_normalized * x_normalized / U256::from(10).pow(U256::from(18)) + 
                  y_normalized * y_normalized / U256::from(10).pow(U256::from(18));
...
impl AutomatedMarketMakerFactory for UniswapV2Factory {
  ...
  fn create_pool(&self, log: Log) -> Result<AMM, AMMError> {
      ...
      Ok(AMM::UniswapV2VariantPool(UniswapV2Pool {
          address: event.pair,
      ...
      P: Provider<N> + Clone,
  {
      ...
      async move {
          ...
          Ok(pairs
              .into_iter()
              .map(|pair| {
                  AMM::UniswapV2VariantPool(UniswapV2Pool {
                      address: pair,
                      factory_address: self.address,
...
mod tests {
  ...
  fn test_get_amount_out() {
      ...
      for fee in fees {
          ...
          let res = pool.get_amount_out(amount_in, reserve_in, reserve_out, pool.token_a.address);
